...
## ۱۰. رفع اشکال: خطای `mise ... No GitHub artifact attestations found`

سیستم Build جدید Railway (Railpack) برای نصب پایتون از ابزار `mise` استفاده می‌کند و گاهی برای یک نسخه‌ی خاص (مثل `3.12.4`) گواهی Attestation گیت‌هاب پیدا نمی‌کند و Build با این خطا شکست می‌خورد:

```
mise ERROR Failed to install core:python@3.12.4: No GitHub artifact attestations found
```

این پروژه از قبل با دو راه برای جلوگیری از این مشکل تنظیم شده:

1. **`runtime.txt`** فقط `python-3.12` (بدون پچ نسخه‌ی دقیق) است تا Railway بتواند یک نسخه‌ی موجود و سالم را انتخاب کند.
2. **`mise.toml`** با تنظیم `python.github_attestations = false` این مرحله‌ی اعتبارسنجی اختیاری را غیرفعال می‌کند (بدون تأثیر بر بررسی checksum فایل دانلودشده).

اگر باز هم با این خطا مواجه شدید:

- در Railway → Variables، متغیر `MISE_PYTHON_GITHUB_ATTESTATIONS=false` را اضافه کنید (معادل تنظیم بالا ولی از طریق Environment Variable).
- یا نسخه‌ی دقیق‌تری در `runtime.txt` امتحان کنید (مثلاً `python-3.12.7` یا جدیدترین پچ موجود).
